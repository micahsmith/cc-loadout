...
- Skills that write file artifacts MUST try to resolve save location using the same convention.
  These skills MUST use `mktemp` or a system temp directory ONLY inside a bundled script. Instructions on file path
  resolution MUST be duplicated inline for any skill that writes files. Any change to this
  convention MUST be synced across all skills. The order for resolving save location is as follows:
